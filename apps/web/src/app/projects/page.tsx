...
const LEDGER_ORDER = ['fact', 'decision', 'source', 'gotcha', 'check', 'access'];
...
/** The project's compact, deduped memory ledger - what tasks have done and learned. */
function LedgerView({ entries }: { entries: LedgerEntry[] }) {
  if (entries.length === 0) {
    return (
      <div>
        <p className="mb-1 text-xs font-medium text-neutral-400">Project memory</p>
        <p className="text-xs text-neutral-500">
          Empty for now - as tasks run, what they do and learn accretes here (compact and
          deduped), and seeds future agents so they skip redundant work.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <p className="mb-2 text-xs font-medium text-neutral-400">Project memory ({entries.length})</p>
      <div className="space-y-3">
        {cats.map((cat) => {
